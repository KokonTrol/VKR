        ...
        try:
            ...
            for index, row in group.iterrows ():
                ...
                if temp_row == "" or temp_row != row["ФИО студента"]:
                    ...
                    results.loc[-1] = [temp_row, row["Команда"], row["Итог ТУ"], GetInfoLose(row["Итог ТУ"])] + [""]*(len(results.columns)-5)
...
def Convert(file_name):
    ...
    if "Название встречи" in xls_array[0].columns:
        return _ConvertEveryRow(xls_array[0])
    else:
        return _ConvertOneRow(xls_array[0])
    #преобразование
